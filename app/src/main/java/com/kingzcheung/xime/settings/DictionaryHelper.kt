...
object DictionaryHelper {
    ...
    fun collectEntries(rootDict: String, readDict: (String) -> String?): List<DictEntry> =
        collectEntriesWithTableCount(rootDict, readDict).first

    /**
     * 同 [collectEntries]，但额外返回实际读取到的词库表数量（不含缺失的表）。
     * 供「方案词库」页展示"N 个词库 · M 条词条"。
     */
    fun collectEntriesWithTableCount(
        rootDict: String,
        readDict: (String) -> String?
    ): Pair<List<DictEntry>, Int> {
        val out = mutableListOf<DictEntry>()
    ...
    fun loadDictionary(context: Context, schemaId: String): List<DictEntry> =
        loadDictionaryWithTables(context, schemaId).first

    /** 同 [loadDictionary]，同时返回实际读取到的词库表数量。 */
    fun loadDictionaryWithTables(context: Context, schemaId: String): Pair<List<DictEntry>, Int> {
        val dictName = SchemaManager.getReferencedDictName(context, schemaId) ?: schemaId
        val dir = SchemaManager.getRimeDir(context)
        return try {
            val rimeDir = dir.absolutePath
            collectEntriesWithTableCount(dictName) { name ->
                // 词库既可位于 rime 根目录，也可位于 dicts/ 子目录（万象）
                val candidates = listOf(File(rimeDir, "$name.dict.yaml"), File(rimeDir, "dicts/$name.dict.yaml"))
